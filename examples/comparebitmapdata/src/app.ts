import { Surface } from '@flighthq/oop';
...
function createCheckers(color1: number, color2: number, tileSize = 8): Surface {
  const img = new Surface(IMG_SIZE, IMG_SIZE, color1);
  for (let y = 0; y < IMG_SIZE; y++) {
    for (let x = 0; x < IMG_SIZE; x++) {
      if (((Math.floor(x / tileSize) + Math.floor(y / tileSize)) & 1) === 1) {
        img.setPixel32(x, y, color2);
      }
    }
  }
  return img;
...
function createNoise(seed: number): Surface {
  const img = new Surface(IMG_SIZE, IMG_SIZE);
  let s = seed >>> 0;
...
function createBall(color: number, alpha = 255): Surface {
  ...
  for (let y = 0; y < IMG_SIZE; y++) {
    for (let x = 0; x < IMG_SIZE; x++) {
      ...
      const dy = y + 0.5 - cy;
      if (dx * dx + dy * dy <= r * r) {
        img.setPixel32(x, y, ((alpha & 0xff) * 0x1000000 + (color & 0xffffff)) >>> 0);
      }
    }
  }
  return img;
...
function createRect(color: number, inset = 4): Surface {
  const img = new Surface(IMG_SIZE, IMG_SIZE);
  img.fillRect(inset, inset, IMG_SIZE - inset * 2, IMG_SIZE - inset * 2, color);
  return img;
}
...
sources[9].img = sources[4].img!.clone();
...
for (let row = 0; row < n; row++) {
  ...
  for (let col = 0; col < n; col++) {
    ...
    if (rowImg === null) {
      ...
      drawCell(ctx, canvas, x + 2, y + 2, rowImg.compare(colImg), false);
    }
  }
}
