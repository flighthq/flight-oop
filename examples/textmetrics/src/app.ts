import type { TextFormat, TextLayoutResult } from '@flighthq/oop';
import { Application, DisplayContainer, RichText, Shape, TextLayout } from '@flighthq/oop';
...
const textField = new RichText();
textField.x = TEXT_X;
textField.y = TEXT_Y;
textField.border = true;
textField.borderColor = 0x000000;
textField.defaultTextFormat = format;
textField.width = FIELD_W;
textField.height = FIELD_H;
...
const result = TextLayout.compute({
  formatRanges: [{ format, start: 0, end: TEXT.length }],
  height: FIELD_H,
  multiline: true,
  text: TEXT,
  width: FIELD_W,
...
const vizBg = new Shape();
vizBg.x = TEXT_X - BUFFER;
vizBg.y = TEXT_Y - BUFFER;
vizBg.beginFill(0xe0e0e0, 1);
vizBg.drawRect(0, 0, bmpW, bmpH);

const vizLines = new Shape();
...
outText.text = buildMetricsString(TEXT_X, TEXT_Y, FIELD_W, FIELD_H, textWidth, textHeight, tlm, result);

// ── Lorem ipsum section ────────────────────────────────────────────────────

const whiteBg = new Shape();
whiteBg.x = 0;
whiteBg.y = 250;
whiteBg.beginFill(0xffffff, 1);
whiteBg.drawRect(0, 0, 200, 200);

const loremText = new RichText();
loremText.x = 0;
loremText.y = 250;
loremText.width = 200;
loremText.height = 200;
loremText.multiline = true;
...
root.addChild(vizLines);
root.addChild(whiteBg);
root.addChild(loremText);
...
function fillRect(shape: Shape, x: number, y: number, w: number, h: number, color: number): void {
  ...
  shape.drawRect(x, y, Math.max(w, 1), Math.max(h, 1));
}
